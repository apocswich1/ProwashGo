...
const Results = props => {
  ...
  const handleSelectAll = event => {
    const selectedCustomers = event.target.checked
      ? drivers.map(customer => customer.id)
      : [];
          ...
          <PerfectScrollbar>
            <div className={classes.inner}>
              <Table>
                <TableHead>
                  <TableRow>
                    <TableCell padding="checkbox">
                      <Checkbox
                        checked={selectedCustomers.length === drivers.length}
                        color="primary"
                        indeterminate={
                          selectedCustomers.length > 0 &&
                          selectedCustomers.length < drivers.length
                        }
                ...
                <TableBody>
                  {drivers.slice(0, rowsPerPage).map(customer => (
                    <TableRow
                      hover
                      key={customer.id}
