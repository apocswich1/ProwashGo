...
const Invoices = props => {
  ...
  const [paidInvoices, paidTotal] = getSum(rate.invoices, 'paid');
  const [draftInvoices, draftTotal] = getSum(rate.invoices, 'draft');
  const [dueInvoices, dueTotal] = getSum(rate.invoices, 'due');
  const [refundedInvoices, refundedTotal] = getSum(
    rate.invoices,
    'refunded'
  );
  const [incomeInvoices, incomeTotal] = getSum(rate.invoices, 'income');
        ...
        <Table>
          <TableBody>
            <TableRow>
              ...
              <TableCell>
                ...
                <div>
                  <Label
                    color={
                      rate.autoCC ? colors.green[600] : colors.red[600]
                    }
                  >
                    {rate.autoCC ? 'YES' : 'NO'}
                  </Label>
            ...
            <TableRow selected>
              ...
              <TableCell>
                {paidInvoices.length} ({rate.currency}
                {paidTotal})
              </TableCell>
            </TableRow>
            <TableRow>
              ...
              <TableCell>
                {draftInvoices.length} ({rate.currency}
                {draftTotal})
              </TableCell>
            </TableRow>
            <TableRow selected>
              ...
              <TableCell>
                {dueInvoices.length} ({rate.currency}
                {dueTotal})
              </TableCell>
            </TableRow>
            <TableRow>
              ...
              <TableCell>
                {refundedInvoices.length} ({rate.currency}
                {refundedTotal})
              </TableCell>
            </TableRow>
            <TableRow selected>
              ...
              <TableCell>
                {incomeInvoices.length} ({rate.currency}
                {incomeTotal})
              </TableCell>
            </TableRow>
          </TableBody>
