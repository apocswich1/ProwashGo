  ...
  const fetchCustomers2 = () => {
    ...
    .then(snapshot => {
      ...
      snapshot.forEach(async doc => {
        ...
        console.log(doc.franchiseID);
        console.log(doc.venueID);
        let todo = await firebase.firestore().collection('franchise').doc(doc.franchiseID)
        .collection('venues').doc(doc.venueID).get();
        let result = await todo.doc.map(item => { return item.data() });
