  ...
  const fetchCustomers2 = () => {
    ...
    .then(async snapshot => {
      ...
      for(var i =0; i < snapshot.docs.length; i++){
        ...
        let todo = await firebase.firestore().collection('franchises').doc(snapshot.docs[i].data().franchiseID)
        .collection('venues').doc(snapshot.docs[i].data().venueID).get();
        let result = todo.data();
        let dato = snapshot.docs[i].data();
        dato.sucursal = result.name;
        posts.push(dato);
        console.log(dato);
      }
      // snapshot.forEach(async doc => { 
      //   console.log(doc.data().franchiseID);
      //   console.log(doc.data().venueID);
      //   let todo = await firebase.firestore().collection('franchises').doc(doc.data().franchiseID)
      //   .collection('venues').doc(doc.data().venueID).get();
      //   let result = todo.data();
      //   console.log(result);
      //   doc.data().sucursal = result.name; 
      //   posts.push(doc.data());
      // });
      // console.log(posts)
    })
